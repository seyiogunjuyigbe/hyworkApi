...
export const assignThisJob = (req,res)=>{
    Organization.findOne({urlname}).populate("employees", "email username -_id").exec((err,org)=>{
        if(err) return response.error(res,500,err.message)
            ...
            Job.findById(req.params._id, (err,thisJob)=>{
                if(err) return response.error(res,500,err.message);
                else if(!thisJob) return response.error(res,404,'Requested job not found');
                else if(thisJob.createdBy !==req.user.username) return response.error(res,403,'You are nit authoeized to assign this job');
                else {
