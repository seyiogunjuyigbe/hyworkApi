...
export const changeCaseStatus = (req,res)=>{
    Organization.findOne({urlname:req.params.urlname}).populate("employees").exec((err,org)=>{
        if(!org.cases.includes(req.params.case_id)) return response.error(res,404,'Case not found');
            ...
            Case.findById(req.params.case_id, (err,updatedCase)=>{            
                 if(err){return response.error(res,500,err.message)}
               else if(!updatedCase)return response.error(res,404,'Case not found');
                else if (updatedCase.createdBy !== req.user.username){
                    ...
                    updatedCase.save((err,done)=>{
                      if(err){return response.error(res,500,err.message)}
                    else return res.status(200).redirect(`/org/${org.urlname}/case/${updatedCase._id}/view`)
                    });
                }
            })
        }
