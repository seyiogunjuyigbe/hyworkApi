...
export const createCase = (req, res) => {
    const { TenantOrganization, Case, User } = req.dbModels;
    TenantOrganization.findOne({ urlname: req.params.urlname }).populate('employees', "username email -_id")
        .then((org => {
            var list = [];
            var emailList = []
...
export const respondToCase = (req, res) => {
    const { TenantOrganization, Case, Comment } = req.dbModels;
    TenantOrganization.findOne({ urlname: req.params.urlname }).populate("employees").exec((err, org) => {
        if (!org.cases.includes(req.params.case_id)) return response.error(res, 404, 'Case not found');
            ...
            Case.findById(req.params.case_id).populate('comments').exec((err, thisCase) => {
                if (err) response.error(res, 500, err.message)
                    ...
                    Comment.create({
                        ...req.body, sender: req.user, sentFor: thisCase,
...
export const fethThiscase = (req, res) => {
    const { TenantOrganization, Case } = req.dbModels;
    TenantOrganization.findOne({ urlname: req.params.urlname }).populate("employees").exec((err, org) => {
        if (!org.cases.includes(req.params.case_id)) return response.error(res, 404, 'Case not found');
        else {
