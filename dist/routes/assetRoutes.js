...
router.post('/:urlname/asset/new', [check('title').isString().not().isEmpty().withMessage('This asset needs a title'), check('description').isString().withMessage('Pleade enter a valid string as description'), check('acquiredBy').isString().not().isEmpty().withMessage('Please specify the recipient of this asset')], validate, _middleware.authUser, _organization.LoggedUserisAdmin, _asset.createAsset);
router.post('/:urlname/asset/:asset_id/update', [check('title').isString().not().isEmpty().withMessage('This asset needs a title'), check('description').isString().withMessage('Pleade enter a valid string as description'), check('acquiredBy').isString().not().isEmpty().withMessage('Please specify the recipient of this asset')], validate, _middleware.authUser, _organization.LoggedUserisAdmin, _asset.modifyAsset);
router.get("/:urlname/asset/:asset_id/delete", _middleware.authUser, _organization.LoggedUserisAdmin, _asset.deleteAsset);
